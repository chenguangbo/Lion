...
<%@ include file="/jsp/header.jsp" %>		
		...
		<div class="container">
			<c:if test="${empty cart.cartItems }">
				<div class="col-md-12">购物车中暂无数据</div>
			</c:if>
					...
					<table class="table table-bordered">
						...
						<c:forEach items="${cart.cartItems}" var="item">	
							<tr class="active">
								...
								<td>
									<a href="javascript:void(0)" class="delete" id="${item.product.pid}")>删除</a>
								</td>
							</tr>
						  </c:forEach>
						</tbody>
...
$(function(){
	$(".delete").click(function(){
		if(confirm("忍心删除我吗?")){
			//向Server发送请求,将当前商品id发送到服务端
			location.href="${pageContext.request.contextPath}/CartServlet?method=removeCartItem&pid="+this.id;
		}
	});
});
</script>
